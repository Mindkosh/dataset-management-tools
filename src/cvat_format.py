...
class Annotation:
    def __init__(self, label_id, points, rotation=None):
        self.label = label_id
        self.points = points
    ...
    for child in root_annotations:
        if(child.tag == "image"):
            ...
            for ind, ann in enumerate(child):
                ...
                rotation = float(ann.attrib["rotation"]) if "rotation" in ann.attrib else None
                ann = Annotation( label_id, points, rotation) 
